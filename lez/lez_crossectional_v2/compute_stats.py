...
for sample in samples:
    ...
    save_data = through_keepers(
        dataset[dataset['dataset'] == sample][[x for x in dataset.columns if x not in [x for x in excludeds[sample] if x != targets[sample]]]],
        sample)

    save_data.to_csv(g.format(sample), index=False)
    ...
    stated.to_csv(d.format(sample, 'stated'))
