...
from load_stats import compute_all_valued_themselves, compute_all_stated, performances_all_valued_themselves, \
    all_stated_over_performances, load_static_measures, get_hist
...
for sa in samples:
    ...
    drop_mask = pd.isna(
        joint[['R-squared adjusted measure__train', 'R-squared adjusted measure__test', 'SMAPE measure__train', 'SMAPE measure__test']])
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        html.Div([
            dcc.Dropdown(['MAXDEPTH', 'SLEAVES', 'MAXSAMPLES', 'CRITERION', 'MODEL', 'LAG', 'PRESELECT', 'NO'],
...
def update_correlations_over_values_themselves(sample, n_lags):
    data_correlations_over_values_themselves = compute_all_valued_themselves(sample=sample, n_lags=n_lags).to_dict(
        'records')
...
def update_performances_over_values_themselves(sample, n_lags):
    data_performances_over_values_themselves = performances_all_valued_themselves(sample=sample, n_lags=n_lags).to_dict(
        'records')
...
def update_correlations_stats_over_performances(sample, n_lags):
    data_correlations_stats_over_performances = all_stated_over_performances(sample=sample, n_lags=n_lags).to_dict(
        'records')
...
def update_scatter_measures_r2adj_sample(sample, scatter_measure, colorizer):
    ...
    if colorizer == 'MAXDEPTH':
        ...
        cozy_usmrs = jj_usmrs['lag'].apply(func=lambda x: 'big_lag' if x == 40 else 'small_lag').values
        vozy_usmrs = jj_usmrs['lag'].apply(func=lambda x: 1 if x == 40 else 0).values
    ...
    mask_drop = jj_usmrs[["measure_train", "measure_test"]].iloc[vozy_usmrs == 0].isna().any(axis=1)
    trend0_estimator_usmrs = LinearRegression()
    trend0_estimator_usmrs.fit(X=jj_usmrs["measure_train"].values[vozy_usmrs == 0][~mask_drop].reshape(-1, 1),
                               y=jj_usmrs["measure_test"].values[vozy_usmrs == 0][~mask_drop])
    ...
    if 1 in vozy_usmrs:
        mask_drop = jj_usmrs[["measure_train", "measure_test"]].iloc[vozy_usmrs == 1].isna().any(axis=1)
        trend1_estimator_usmrs = LinearRegression()
        trend1_estimator_usmrs.fit(X=jj_usmrs["measure_train"].values[vozy_usmrs == 1][~mask_drop].reshape(-1, 1),
                                   y=jj_usmrs["measure_test"].values[vozy_usmrs == 1][~mask_drop])
...
def update_scatter_measures_r2adj_all(sample, scatter_measure, colorizer):
    ...
    if colorizer == 'MAXDEPTH':
        ...
        cozy_usmra = jj_usmra['lag'].apply(func=lambda x: 'big_lag' if x == 40 else 'small_lag').values
        vozy_usmra = jj_usmra['lag'].apply(func=lambda x: 1 if x == 40 else 0).values
    elif colorizer == 'PRESELECT':
    ...
    mask_drop = jj_usmra[["measure_train", "measure_test"]].iloc[vozy_usmra == 0].isna().any(axis=1)
    trend0_estimator_usmra = LinearRegression()
    trend0_estimator_usmra.fit(X=jj_usmra["measure_train"].values[vozy_usmra == 0][~mask_drop].reshape(-1, 1),
                               y=jj_usmra["measure_test"].values[vozy_usmra == 0][~mask_drop])
    ...
    if 1 in vozy_usmra:
        mask_drop = jj_usmra[["measure_train", "measure_test"]].iloc[vozy_usmra == 1].isna().any(axis=1)
        trend1_estimator_usmra = LinearRegression()
        trend1_estimator_usmra.fit(X=jj_usmra["measure_train"].values[vozy_usmra == 1][~mask_drop].reshape(-1, 1),
                                   y=jj_usmra["measure_test"].values[vozy_usmra == 1][~mask_drop])
...
def update_scatter_measures_smape_sample(sample, scatter_measure, colorizer):
    ...
    if colorizer == 'MAXDEPTH':
        ...
        cozy_usmss = jj_usmss['lag'].apply(func=lambda x: 'big_lag' if x == 40 else 'small_lag').values
        vozy_usmss = jj_usmss['lag'].apply(func=lambda x: 1 if x == 40 else 0).values
    elif colorizer == 'PRESELECT':
    ...
    mask_drop = jj_usmss[["measure_train", "measure_test"]].iloc[vozy_usmss == 0].isna().any(axis=1)
    no_zeros = (~mask_drop).sum() > 0
    if no_zeros:
        trend0_estimator_usmss = LinearRegression()
        trend0_estimator_usmss.fit(X=jj_usmss["measure_train"].values[vozy_usmss == 0][~mask_drop].reshape(-1, 1),
                                   y=jj_usmss["measure_test"].values[vozy_usmss == 0][~mask_drop])
    ...
    mask_drop = jj_usmss[["measure_train", "measure_test"]].iloc[vozy_usmss == 1].isna().any(axis=1)
    no_ones = (~mask_drop).sum() > 0
    if (1 in vozy_usmss) and no_ones:
        trend1_estimator_usmss = LinearRegression()
        trend1_estimator_usmss.fit(X=jj_usmss["measure_train"].values[vozy_usmss == 1][~mask_drop].reshape(-1, 1),
                                   y=jj_usmss["measure_test"].values[vozy_usmss == 1][~mask_drop])
    ...
    fig_usmss = go.Figure()
    if (~jj_usmss[["measure_train", "measure_test"]].isna().any(axis=1)).sum() > 0:
...
def update_scatter_measures_smape_all(sample, scatter_measure, colorizer):
    ...
    if colorizer == 'MAXDEPTH':
        ...
        cozy_usmsa = jj_usmsa['lag'].apply(func=lambda x: 'big_lag' if x == 40 else 'small_lag').values
        vozy_usmsa = jj_usmsa['lag'].apply(func=lambda x: 1 if x == 40 else 0).values
    elif colorizer == 'PRESELECT':
    ...
    mask_drop = jj_usmsa[["measure_train", "measure_test"]].iloc[vozy_usmsa == 0].isna().any(axis=1)
    no_zeros = (~mask_drop).sum() > 0
    trend0_estimator_usmsa = LinearRegression()
    trend0_estimator_usmsa.fit(X=jj_usmsa["measure_train"].values[vozy_usmsa == 0][~mask_drop].reshape(-1, 1),
                               y=jj_usmsa["measure_test"].values[vozy_usmsa == 0][~mask_drop])
    ...
    mask_drop = jj_usmsa[["measure_train", "measure_test"]].iloc[vozy_usmsa == 1].isna().any(axis=1)
    no_ones = (~mask_drop).sum() > 0
    if 1 in vozy_usmsa:
        mask_drop = jj_usmsa[["measure_train", "measure_test"]].iloc[vozy_usmsa == 1].isna().any(axis=1)
        trend1_estimator_usmsa = LinearRegression()
        trend1_estimator_usmsa.fit(X=jj_usmsa["measure_train"].values[vozy_usmsa == 1][~mask_drop].reshape(-1, 1),
                                   y=jj_usmsa["measure_test"].values[vozy_usmsa == 1][~mask_drop])
    ...
    fig_usmsa = go.Figure()
    if (~jj_usmsa[["measure_train", "measure_test"]].isna().any(axis=1)).sum() > 0:
    ...
    if (1 in vozy_usmsa) and no_ones:
        fig_usmsa.add_trace(go.Scatter(
            x=trend1_x_usmsa,
            y=trend1_y_usmsa,
            line=dict(color='orange'),
