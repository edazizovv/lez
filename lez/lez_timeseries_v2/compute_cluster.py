...
for sample in samples:
    ...
    if 'TS6' in sample:
        ...
        print('subsampling TS6: 10% taken')
    ...
    # if you have a ValueError here, check [pynndescent == 0.5.8]
    data_d5 = reducer5.fit_transform(X=data_whitened)
    data_d2 = reducer2.fit_transform(X=data_whitened)

    # reducer5.fit(X=data_whitened)
    # reducer2.fit(X=data_whitened)

    # data_d5 = reducer5.transform(X=data_whitened)
    # data_d2 = reducer2.transform(X=data_whitened)
    ...
    for j in range(len(clus)):
        ...
        data_d5 = reducer5.fit_transform(X=data_whitened)
        print('UMAP-5 transformed')
        data_d2 = reducer2.fit_transform(X=data_whitened)
        print('UMAP-2 transformed')
        for n in n_clus:

            clusterer = clu(n_clusters=n)

            clusters = clusterer.fit_predict(X=data_d5)
            print('KMEANS-5 done: nc={0}'.format(n))

            result = pandas.DataFrame(data={'x': data_d2[:, 0], 'y': data_d2[:, 1], 'c': clusters})

            if not os.path.isdir(dd.format(sample)):
                os.mkdir(dd.format(sample))
            result.to_csv(d.format(sample, '{0}_{1}'.format(method, n), n_lags))
