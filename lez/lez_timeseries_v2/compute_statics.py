...
from load_stats import get_corr
...
for sample in samples:
    ...
    columns = ['cond_fro', 'cond_inf', 'cond_p1', 'cond_p2', 'cond_eval', 'det']
    source = []
    cond_fro = []
    cond_inf = []
    cond_p1 = []
    ...
    mx = get_corr(sample=sample, n_lags=n_lags, measure='pearson', direct='direct', treat=False, fill_na=True)

    x_cond_fro = numpy.linalg.cond(x=mx.values, p='fro') / mx.shape[0]
    x_cond_inf = numpy.linalg.cond(x=mx.values, p=numpy.inf) / mx.shape[0]
    x_cond_p1 = numpy.linalg.cond(x=mx.values, p=1) / mx.shape[0]
    x_cond_p2 = numpy.linalg.cond(x=mx.values, p=2) / mx.shape[0]

    x_cond_evalues = numpy.abs(scipy.linalg.eigvals(mx.values))
    x_cond_eval = x_cond_evalues.max() / x_cond_evalues.min()

    x_det = scipy.linalg.det(mx.values)

    source.append('pearson_direct')
    cond_fro.append(x_cond_fro)
    cond_inf.append(x_cond_inf)
    cond_p1.append(x_cond_p1)
    cond_p2.append(x_cond_p2)
    cond_eval.append(x_cond_eval)
    det.append(x_det)

    mx = get_corr(sample=sample, n_lags=n_lags, measure='pearson', direct='partial', treat=False, fill_na=True)
    ...
    mx = get_corr(sample=sample, n_lags=n_lags, measure='kendall', direct='direct', treat=False, fill_na=True)
    ...
    mx = get_corr(sample=sample, n_lags=n_lags, measure='mutual_info', direct='direct', treat=False, fill_na=True)
    ...
    source.append('mutual_info_direct')
    cond_fro.append(x_cond_fro)
    cond_inf.append(x_cond_inf)
    cond_p1.append(x_cond_p1)
    cond_p2.append(x_cond_p2)
    ...
    stated = pandas.DataFrame(data=numpy.array([cond_fro, cond_inf, cond_p1, cond_p2, cond_eval, det]).T,
                              index=source, columns=columns)

    if not os.path.isdir(dd.format(sample)):
        os.mkdir(dd.format(sample))
    stated.to_csv(d.format(sample, 'static', n_lags))
